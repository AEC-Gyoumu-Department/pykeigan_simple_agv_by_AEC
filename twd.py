...
CAM_U1_REAR_ID = 1  # USBcam1 /dev/video1
CAM_U_WIDTH = 320
CAM_U_HEIGHT = 240
CAM_U_FPS = 10
...
camera_front.set(cv2.CAP_PROP_FRAME_WIDTH, CAM_U_WIDTH)
camera_front.set(cv2.CAP_PROP_FRAME_HEIGHT, CAM_U_HEIGHT)
camera_front.set(cv2.CAP_PROP_FPS, CAM_U_FPS)
...
camera_rear = cv2.VideoCapture(CAM_U1_FRONT_ID)
camera_rear.set(cv2.CAP_PROP_FRAME_WIDTH, CAM_U_WIDTH)
camera_rear.set(cv2.CAP_PROP_FRAME_HEIGHT, CAM_U_HEIGHT)
camera_rear.set(cv2.CAP_PROP_FPS, CAM_U_FPS)
# camera.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('H', '2', '6', '4'));
camera_rear.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('Y', 'U', 'Y', 'V'));
camera_rear.set(cv2.CAP_PROP_BUFFERSIZE, 1)
capture_rear = threading_capture(camera_front)
capture_rear.start()
...
def scheduler():
    global cur_state, isPausingLinetrace, isDocking,AGV_direction
    global eI, x, x_old
    ...
    if cur_state == State.STATE_LINE_TRACE:
        twd.run(rpm[0] * AGV_direction, rpm[1] * AGV_direction)  # 速度指令
...
if __name__ == '__main__':
    ...
    GPIO.setup(BUTTON_REAR_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
    ...
    GPIO.add_event_detect(BUTTON_REAR_PIN, GPIO.FALLING, callback=rear_callback, bouncetime=50)
        ...
        while (True):
            ...
            if ids is not None:
                ...
                if 0 == ids[0, 0]:  # id: 0 で右折
                    ...
                    twd = TWD(port_left, port_right, wheel_d=100.6, tread=306.5,
                              button_event_cb=motor_event_cb)  # for try purpose
                    img_stop = cv2.aruco.drawDetectedMarkers(roi_ar, corners, ids, (0, 255, 0))
                    cv2.imwrite("img_stop.jpg", img_stop)

            # (b) 赤ラインマーカーで停止を行う場合
            # red = get_red_moment(img)
            # stopFlag = red[0] # 赤マーカーが存在する場合、True  # The waste Box of the Genba is red, than this part doesnt not work correctly
                    ...
                    if isLineExist:
                        ...
                        if lost_count >= LOST_THRESHOLD:
                            ...
                            if lost_total_count >= LOST_TOTAL_THRESHOLD:
                                ...
                                if cur_state == State.STATE_LINE_TRACE:
                                    twd.move_straight(STOP_AFTER_RPM, -180 * AGV_direction, 5)
                                    # 車輪半回転 180°後退、15秒タイムアウトで前進 hsv3:440->510
